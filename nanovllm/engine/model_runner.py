...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        self.chunked_prefill = getattr(config, "chunked_prefill", False)
        self.world_size = config.tensor_parallel_size
        self.rank = rank
        self.event = event
        self.use_cudagraph = (not self.enforce_eager) and (not self.chunked_prefill)
        ...
        # chunked prefill 模式下先不走 cudagraph，避免旧 decode-only graph 假设出错
        ...
        if self.world_size > 1:
            if rank == 0:
                ...
                try:
                    ...
                    existing_shm.close()
                    existing_shm.unlink()
                    print("Cleaned up existing shared memory: nanovllm")
    ...
    def warmup_model(self):
        ...
        # chunked 模式下允许 max_num_batched_tokens < max_model_len，所以至少 warmup 1 条
        num_seqs = max(
            min(max_num_batched_tokens // max_model_len, self.config.max_num_seqs),
            1,
        )
    ...
    def prepare_model_input(self, seqs: list[Sequence]):
        ...
        for seq_index, seq in enumerate(seqs):
            # 只有本轮正好算到当前序列尾部的 seq，才需要算 logits 并采样
            ...
            # 为本轮新写入 KV 的 token 计算 slot_mapping
        ...
        # 如果 k 总长度大于 q，总说明存在 prefix cache / decode / chunk continuation
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor):
        # chunked prefill 模式下先统一走 eager，避免旧 cudagraph 路径不兼容
        if self.chunked_prefill or self.enforce_eager or input_ids.size(0) > 512:
        ...
        graph_vars["slot_mapping"][: context.slot_mapping.size(0)] = context.slot_mapping

        graph_vars["context_lens"].zero_()
        graph_vars["context_lens"][: context.context_lens.size(0)] = context.context_lens
    ...
    def run(self, seqs: list[Sequence]):
        ...
        seq_need_compute_logits = (
            seq_need_compute_logits.tolist() if self.rank == 0 else None
        )
